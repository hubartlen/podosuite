      ...
      {options.map(([val, txt]: [string, string]) => (
        <button key={val} type="button" onClick={() => onChange(val)}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
            value === val ? 'bg-blue-50 text-blue-700 border-blue-300' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'
          }`}>{txt}</button>
      ))}
    </div>
  </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
        ...
        <div className="mt-4">
          ...
          <textarea value={form.remarques} onChange={e => set('remarques', e.target.value)}
            placeholder="Observations libres..." rows={3}
