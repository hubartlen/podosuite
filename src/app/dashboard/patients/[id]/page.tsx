  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="grid grid-cols-3 gap-5 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-5 col-span-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {patient.telephone && (
              <div>
                <p className="text-xs text-slate-400 mb-1 flex items-center gap-1"><Phone size={10} /> Téléphone</p>
            ...
            {patient.adresse && (
              <div className="col-span-2">
                <p className="text-xs text-slate-400 mb-1 flex items-center gap-1"><MapPin size={10} /> Adresse</p>
                <p className="text-sm font-medium text-slate-800">{patient.adresse}</p>
              </div>
            )}
            {patient.num_secu && (
              <div className="col-span-2">
                <p className="text-xs text-slate-400 mb-1 flex items-center gap-1"><CreditCard size={10} /> N° Sécurité Sociale</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-3xl font-semibold text-slate-800">{bilans?.length ?? 0}</p>
              <p className="text-sm text-slate-500">bilan(s) réalisé(s)</p>
            </div>
            <div>
              <p className="text-3xl font-semibold text-slate-800">{factures?.length ?? 0}</p>
              <p className="text-sm text-slate-500">facture(s) émise(s)</p>
            </div>
            {factures && factures.length > 0 && (
              <div>
                <p className="text-3xl font-semibold text-slate-800">
                  {factures.reduce((s, f) => s + (f.total || 0), 0).toFixed(0)} €
                </p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {historique.length === 0 ? (
          <div className="py-12 text-center text-slate-400 text-sm">Aucun historique pour ce patient</div>
        ) : (
                ...
                <div className="flex-1">
                  ...
                  {item._type === 'bilan' ? (
                    <p className="text-sm text-slate-600">
                      {[
                        item.valgus_calcaneen !== 'absent' && 'Valgus calcanéen',
                        item.semelles_hci && 'HCI',
                        item.semelles_hce && 'HCE',
                        item.talonnette_mm && `Talonnette ${item.talonnette_mm}mm`,
                      ].filter(Boolean).join(' · ') || 'Bilan podologique'}
                    </p>
                  ) : (
                    <p className="text-sm text-slate-600">
                      {item.numero} · {(item.total || 0).toFixed(2)} € · {item.mode_paiement}
                    </p>
                  )}
                </div>
              </div>
