...
export default function DashboardPage() {
  ...
  if (isMobile) {
    return (
      <div style={{padding:'20px 16px', background:'var(--bg)', minHeight:'100%'}}>
        {/* Header */}
        <div style={{marginBottom:20}}>
          <p style={{fontSize:11,fontWeight:500,letterSpacing:'.08em',textTransform:'uppercase',color:'var(--fg-3)',marginBottom:4}}>{today}</p>
          <h1 style={{fontFamily:'var(--font-display)',fontWeight:400,fontSize:32,color:'var(--fg)',margin:0,letterSpacing:'-0.01em'}}>Bonjour, {data.prenom}</h1>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
          {[
            ...
            <div key={i} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:14,padding:'14px 14px'}}>
              ...
              <span style={{fontFamily:'var(--font-display)',fontSize:24,fontWeight:400,color:'var(--fg)',display:'block',marginBottom:4}}>{k.value}</span>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
          {[
            ...
            <Link key={i} href={s.href} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:14,padding:14,display:'flex',flexDirection:'column',gap:6,textDecoration:'none'}}>
              <div style={{width:32,height:32,borderRadius:8,background:'var(--dark)',color:'var(--accent)',display:'grid',placeItems:'center',fontSize:16}}>→</div>
              <div style={{fontSize:13,fontWeight:600,color:'var(--fg)',lineHeight:1.3}}>{s.label}</div>
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:14,overflow:'hidden',marginBottom:16}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 16px',borderBottom:'1px solid var(--line)'}}>
            <span style={{fontSize:11,fontWeight:600,letterSpacing:'.04em',textTransform:'uppercase',color:'var(--fg-3)'}}>Facturation · 7 jours</span>
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:14,overflow:'hidden'}}>
          ...
          {data.recents.map((p:any,i:number) => (
            <Link key={p.id} href={`/dashboard/patients/${p.id}`} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderBottom:i<data.recents.length-1?'1px solid var(--line)':'none',textDecoration:'none'}}>
              <div style={{width:36,height:36,borderRadius:'50%',background:'var(--dark)',color:'var(--accent)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:600,flexShrink:0,fontFamily:'var(--font-display)'}}>
                {p.prenom?.[0]}{p.nom?.[0]}
              </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:20}}>
        {[
          ...
          <div key={i} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:16,padding:'18px 20px',display:'flex',flexDirection:'column',gap:8,boxShadow:'var(--shadow-1)'}}>
            ...
            <span style={{fontFamily:'var(--font-display)',fontSize:32,fontWeight:400,letterSpacing:'-0.01em',color:'var(--fg)'}}>{k.value}</span>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:20}}>
        {[
          ...
          <Link key={i} href={s.href} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:16,padding:14,display:'flex',alignItems:'center',gap:12,textDecoration:'none',boxShadow:'var(--shadow-1)'}}>
            <div style={{width:40,height:40,borderRadius:10,background:'var(--dark)',color:'var(--accent)',display:'grid',placeItems:'center',flexShrink:0,fontSize:18,fontWeight:600}}>→</div>
            <div>
